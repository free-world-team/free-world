#!/usr/bin/env python3
"""Build deterministic three-state Qinglan landmark atlases from approved transparent sheets."""
...
def main() -> None:
    ...
    for landmark in LANDMARKS:
        transparent_path = args.working_dir / f"{landmark}-transparent.png"
        sheet = Image.open(transparent_path).convert("RGBA")
        sheet.putalpha(
            sheet.getchannel("A").point(
        ...
        if unique != STATE_COUNT:
            raise ValueError(f"{landmark} must contain three unique states, got {unique}")
        if margin < FINAL_SAFE_MARGIN - 1:
            raise ValueError(f"{landmark} safe margin {margin} is below {FINAL_SAFE_MARGIN - 1}")
        magenta = visible_magenta(final)
        danger = danger_pixels(final)
        if magenta:
            raise ValueError(f"{landmark} contains {magenta} visible magenta pixels")
        if danger:
            raise ValueError(f"{landmark} contains {danger} exact P0 danger-red pixels")

        source_master_path = args.working_dir / f"{landmark}-source-master.png"
        atlas_master_path = args.working_dir / f"{landmark}-state-atlas-master.png"
        final_path = args.final_dir / f"{landmark}-state-atlas.png"
        ...
        print(
            f"landmark={landmark} source={sheet.size} source_master={source_master.size} "
