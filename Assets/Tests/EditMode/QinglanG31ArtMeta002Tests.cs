...
{
    public sealed class QinglanG31ArtMeta002Tests
    {
        private const string AssetRoot = "Assets/GameAssets/AI/QinglanDemo/ART-META-002";

        private static readonly string[] Inserts =
        {
        ...
        [Test]
        public void ThreeInsertIconsUseStableIdsCenteredImportsAndReleaseEntries()
        {
            for (var index = 0; index < Inserts.Length; index++)
            {
                var name = Inserts[index];
                ...
                var stableId = "qinglan.presentation.insert." + name.Replace('-', '_');
                var sprite = FindSprite(path, stableId);
                Assert.That(sprite, Is.Not.Null, stableId);
                Assert.That(sprite.rect.width, Is.EqualTo(256));
                Assert.That(sprite.rect.height, Is.EqualTo(256));
                Assert.That(sprite.pivot.x, Is.EqualTo(128f).Within(0.01f));
                Assert.That(sprite.pivot.y, Is.EqualTo(128f).Within(0.01f));
                AssertReleaseEntry(path, "qinglan/hub/insert/" + name + "/icon");
            }
        }

        [Test]
        public void ApprovedSourcesMastersAndFinalsMeetAlphaColorAndDistinctnessBudgets()
        {
            ...
            {
                var name = Inserts[index];
                AssertOpaqueSquare(projectRoot, SourcePath(name), 1254);
                AssertTransparentSquare(projectRoot, TransparentPath(name), 1254, 0, 0.20f, 0.75f, null, null);
                AssertTransparentSquare(projectRoot, MasterPath(name), 2048, 127, 0.45f, 0.54f, null, null);
                AssertTransparentSquare(projectRoot, FinalPath(name), 256, 15, 0.45f, 0.54f, colorHashes, grayscaleHashes);
            }

            Assert.That(colorHashes.Count, Is.EqualTo(Inserts.Length));
            Assert.That(grayscaleHashes.Count, Is.EqualTo(Inserts.Length),
                "Insert silhouettes and internal value structures must remain distinct without hue.");
        }

        [Test]
        public void OnlyThreeApprovedFinalInsertIconsAreAddressable()
        {
        ...
        private static string TransparentPath(string name) => AssetRoot + "/working/" + name + "-transparent.png";
        private static string MasterPath(string name) => AssetRoot + "/working/" + name + "-source-master.png";
        private static string FinalPath(string name) => AssetRoot + "/final/" + name + "-icon.png";
